    ...
    if (growthFailed) {
      ...
      const orderedLineages = [...majorLineageIds]
        ...
        .sort((left, right) => {
          ...
          return leftX - rightX || String(left).localeCompare(String(right));
        });
      ...
      allBranchBounds.length = 0;
      allBranchCurves.length = 0;
      for (const branch of recoveredBranches.values()) {
        allBranchBounds.push(...branchBounds(branch));
        allBranchCurves.push({
          branchId: branch.id,
          samples: sampleCubicBezier(branch.curve, { tolerance: 4, maxSubdivisionDepth: 10 }),
        });
      }
      growthFailed = false;
